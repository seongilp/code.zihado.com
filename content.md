...
> 1년 동안 100개의 서비스를 만드는 프로젝트 — 지금까지 102개
...
| 분야 | 프로젝트 수 | 대표작 |
|------|:---:|------|
| 🌐 웹 서비스·대시보드 | 60 | 국회 감시 대시보드, 법령 검색, 실시간 같이보기(Unflix) |
| 🤖 자동화·봇·스크래핑 | 12 | 출석 자동화 봇, 재입고 알림, 로또 자동 구매 |
...
### 공영경기 데이터 수집·AI 예측 · `kra.unflix.tv`
...
### 캠핑나우 · `datalab/kr-camping-now`
전국 캠핑장 3,099곳을 지도에서 조건에 맞게 찾는 앱. 업종(글램핑·카라반·오토캠핑)·입지(해변·계곡·숲)·반려동물 동반·연중운영으로 거른다.
- 한국관광공사 고캠핑 API 전량(3,099곳, 좌표 99.7%)을 캐싱, 실측 채움률로 필터 항목을 골라 업종 99.8%·입지 84%·반려동물 84%만 필터로 쓰고 채움률 낮은 예약링크(46%)는 상세에만 노출
- 지도에 안 내려온 캠핑장도 `⌘K` 커맨드 팔레트로 이름 검색 가능
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · shadcn/ui · MapLibre GL v5 · cmdk · 한국관광공사 고캠핑 API
- **🎓 배울 수 있는 것**: 필드별 실측 채움률을 먼저 재고 그 결과로 필터 항목을 정하는 법 — 채움률 낮은 필드로 거르면 "없다"가 거짓말이 된다 / 지도 뷰포트 밖 데이터를 커맨드 팔레트 검색으로 보완하는 설계

### 도서관나우 · `datalab/kr-library-now`
공공도서관 열람실의 실시간 잔여 좌석을 지도에서 보는 앱. 만석·미제공·갱신 안 됨을 구분해 보여준다.
- 도서관 168곳 중 실시간 좌석을 주는 곳은 123곳(수도권 85%) — 얇은 커버리지를 숨기지 않고 화면에 명시
- 잔여 0(만석)과 결측(미제공)을 타입으로 강제 구분해 같은 색으로 뭉뚱그리지 않음, 정적인 도서관 목록과 60초짜리 좌석 데이터의 캐시 TTL을 분리
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · shadcn/ui · MapLibre GL v5 · 공공도서관 열람실 실시간 정보(행안부 NIA)
- **🎓 배울 수 있는 것**: 만석(값 0)과 미제공(결측)을 타입 수준에서 분리해 실수로 같은 색으로 그리지 않게 막는 법 / 갱신 주기가 다른 데이터(정적 목록 vs 60초 좌석)를 캐시 TTL로 분리하는 설계 / 얇은 커버리지를 숨기지 않고 화면에 정직하게 드러내는 UX

### 코리아뮤지엄 · `datalab/kr-museum-now`
전국 박물관·미술관·전시관·기념관을 지도에서 찾는 앱. "오늘 여는 곳"만 걸러 월요일 휴관 헛걸음을 막는다.
- 한국관광공사 공식 데이터로 전국 박물관·미술관·전시관·기념관을 지도에 표시
- 휴관일을 해석해 "오늘 여는 곳"만 거르고, 판정이 애매하면 "확인 필요"로 정직하게 구분
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · shadcn/ui · MapLibre GL v5 · cmdk · 한국관광공사 TourAPI
- **🎓 배울 수 있는 것**: 휴관일처럼 애매한 데이터를 "열림/닫힘/확인 필요" 세 값으로 나눠 오판을 줄이는 법 / 형제 프로젝트(캠핑나우·도서관나우)와 같은 지도+필터+커맨드팔레트 구조를 재사용하는 설계

### 차 유지비 손익분기 · `tesla`
GV70(가솔린)와 Tesla Model Y(전기)의 총소유비용을 슬라이더로 비교해 몇 년 타야 전기차가 더 싸지는지 계산하는 계산기.
- 연료비·전기요금·보험료·자동차세·정비비·차값·감가상각을 슬라이더로 입력해 총소유비용 계산, 두 차종 비용 곡선이 교차하는 손익분기 시점을 표시
- 빌드 도구 없는 단일 HTML 파일로 제작, Vercel에 정적 배포
- **스택**: HTML · CSS · Vanilla JavaScript
- **🎓 배울 수 있는 것**: 프레임워크 없이 슬라이더 다수를 실시간으로 연동해 계산·차트를 그리는 법 / 총소유비용처럼 여러 변수가 얽힌 의사결정을 손익분기 그래프 하나로 압축해 보여주는 UX

### 보도블록 · `paving-tetris`
줄을 지우는 대신 예쁜 패턴으로 보도블록을 까는 게 목적인 테트리스 변형 게임.
- 헤링본·바구니짜기·완성 줄·대각선 악센트 패턴을 판정해 점수를 매기고, 구멍이 생기면 감점
- I형·L형·2×2 판석 조각을 셀 단위로 충돌 판정하되, 렌더링에서는 톱니 실루엣으로 이웃과 맞물리게 그림
- 키보드·터치 양쪽을 지원하는 모바일 우선(390px) 레이아웃, 빌드 도구 없는 순수 HTML/CSS/JS + Canvas 2D
- **스택**: HTML · CSS · Vanilla JavaScript · Canvas 2D
- **🎓 배울 수 있는 것**: 패턴 판정 로직을 DOM과 분리한 순수 함수로 짜서 node --test로 검증하는 법 / 충돌 판정(셀 단위 직사각)과 시각적 표현(물결 실루엣)을 분리해 렌더링만 바꾸는 구조

### 광명나우 · `datalab/gm-now`
광명시 지금 날씨·미세먼지·버스 도착·주차장을 한 화면에서 보는 지역 밀착형 "나우" 패밀리 앱.
- 기상청 초단기실황·예보로 광명시청 격자 기준 현재 날씨와 6시간 예보, 에어코리아 소하동·철산동 두 측정소 미세먼지를 색+텍스트 병기로 표시
- 경기버스정보시스템(GBIS) 실시간 버스 도착정보와 광명시 주차장 28곳을 거리순으로 안내
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · 기상청 단기예보 API · 에어코리아 API · GBIS
- **🎓 배울 수 있는 것**: 정부 API 3개가 성공·실패를 서로 다른 필드로 알리는 문제를 공통 파서로 흡수하는 법 / 게이트웨이 버스트 장애가 잦은 API에 "3시간 이내 마지막 성공값" stale 폴백을 설계하는 법

### 업추비나우 · `datalab/kr-expense-now`
전국 243개 지자체 업무추진비를 랭킹·지도로 비교하는 앱. 지방재정365 결산 데이터 기반.
- 지방재정365 데이터셋 5개를 결합해 243개 지자체의 업무추진비 결산액·절감률·1인당 지출을 계산, 카카오지도에서 시군구 경계 hover 하이라이트로 시각화
- 시도별·전국 랭킹과 7개 지표 정렬, 연도별 타임라인 슬라이더, 지자체 243곳 상세 페이지(추이 차트) 정적 생성
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · shadcn/ui · 카카오지도 JS SDK · 지방재정365
- **🎓 배울 수 있는 것**: data.go.kr "LINK" 유형 데이터를 활용신청 없이 원본 소스에서 직접 받아오는 우회 경로 찾기 / 연도별로 정의가 뒤집힌 원본 지표를 원시 데이터로 직접 재계산해 신뢰성을 확보하는 법

### 공원나우 · `datalab/kr-park-now`
전국 도시공원 17,377곳을 지도에서 찾는 앱. 내 위치 기준 가까운 순·넓은 순, "지금 갈 만한 공원" 즉시 추천.
- 행안부 전국도시공원정보표준데이터 17,377건을 정적 스냅샷으로 수집·정규화(시도 오타·행정구역 개편까지 흡수)
- MapLibre GL로 전량 지도 표시, 반경 1km 안 가장 넓은 공원을 즉시 추천하고 면적을 축구장·농구장 개수로 환산
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · shadcn/ui · MapLibre GL v5 · 행안부 표준데이터
- **🎓 배울 수 있는 것**: 유일키인 줄 알았던 필드가 실제로는 중복되는 공공데이터를 이름+좌표로 정제하는 패턴 / 17만 건급 지도 데이터를 브라우저 메모리 계산으로 처리해 런타임 API 호출 0회로 만드는 설계

### 그리모어 · `vfx/grimoire`
자연어로 절차적 VFX를 만들고 링크 하나로 공유하는, 서버도 계정도 없는 이펙트 에디터.
- 오픈소스 VFX 샌드박스(Elemental Sandbox) 위에 파라미터 diff를 URL로 인코딩하는 공유 링크 기능 추가
- "불덩이를 더 크게" 같은 자연어 요청을 Claude가 실제 슬라이더 패치로 변환해 화면에 즉시 반영, 잘못된 응답은 검증된 병합으로 걸러 시각적 실수로만 국한
- **스택**: Three.js · GLSL · Vite · Claude API
- **🎓 배울 수 있는 것**: LLM이 "이미 존재하는 슬라이더"만 조작하게 제한해 실패 모드를 안전한 범위로 좁히는 설계 / 서버·DB 없이 파라미터 diff만 URL에 담아 상태를 공유하는 패턴

### GTA 서울 · `datalab/gta-seoul`
실제 강남 지형 데이터로 만든 브라우저 3D 드라이빙 데모.
- Overture Maps 건물 18,075동·도로 5,023개를 압출해 실제 강남을 3D로 재현
- WASD로 자동차를 몰고 건물 충돌·3인칭 추적 카메라·미니맵 지원, 선릉로 같은 실제 도로명과 무역센터·파르나스타워 등 랜드마크 표시
- **스택**: three.js · Vite · DuckDB (Overture Maps 추출) · Vercel
- **🎓 배울 수 있는 것**: 오픈 지리 데이터를 3D 도시로 바꾸는 파이프라인(footprint + 높이 → 압출) / 수만 개 메쉬를 병합해 드로우콜을 줄이는 웹 3D 성능 기초

### 플라이트나우 · `datalab/kr-flight-now`
대한민국 상공과 전 세계 항공기를 실시간으로 보여주는 지도 (FlightRadar24 스타일).
- 무료 ADS-B 3개 망을 합쳐 항공기 위치를 10초마다 갱신, 회전하는 기종별 아이콘으로 표시
- 항공기를 누르면 편명·기종·등록번호·고도·속도와 출발지→도착지 대권항로·진행률 표시, 줌아웃하면 전 세계 1만 3천여 대를 격자 솎아내기로 균등 렌더
- **스택**: Next.js 16 · MapLibre GL v5 · adsb.fi·adsb.lol·OpenSky · hexdb.io · Vercel
- **🎓 배울 수 있는 것**: 여러 무료 데이터 소스를 합쳐 커버리지를 끌어올리는 법(합집합으로 +47%) / 서버 캐시로 상류 API를 보호하면서 실시간처럼 보이게 만들기(보간 이동)

### 해외취업나우 · `datalab/kr-worldjob-now`
지금 채용 중인 해외 기업과 지금 신청 가능한 해외취업 준비 일정을 모은 사이트.
- 채용 진행 중인 기업 267곳·공고 626건·마감 임박 83건을 국가·업종으로 탐색
- 해외연수·취업설명회·아카데미 중 오늘 이후 일정만 마감 임박순으로 월드잡플러스 원문 연결, 과거 채용 4,283건을 국가·직종·비자·어학 요건으로 분석해 준비 방향 제시(과거 기록임을 명시)
- **스택**: Next.js 16 (App Router) · TypeScript · Tailwind CSS v4 · 한국산업인력공단 월드잡플러스 OpenAPI 32종 · Vercel
- **🎓 배울 수 있는 것**: 공개 데이터가 "살아있는지" 먼저 재고 제품 방향을 정하는 법 — 마감된 공고를 살아있는 척 보여주지 않기 / XML 공공 API를 수집 스크립트로 스냅샷 만들어 런타임 호출 0으로 서비스하기

### Remedy · `remedy`
싱잉볼 명상, 데일리 퀘스트와 주간 리더보드를 제공하는 한국어 명상 웹 서비스.
- 싱잉볼 명상 세션을 진행하고 완료 여부를 서버에 기록해 데일리 퀘스트와 연동
- Next.js 게이트웨이가 서버 전용 자격증명으로만 호출 가능한 비공개 Cloudflare Worker API에 명상 기록을 위임
- 완료한 명상 기록으로 주간 리더보드를 계산하고, 옵트인한 사용자만 순위에 노출
- **스택**: Next.js 16 · Cloudflare Workers · Cloudflare D1 · Drizzle ORM
- **🎓 배울 수 있는 것**: 브라우저는 same-origin API만 호출하고 서버가 비공개 Worker에 인증서를 붙여 대신 호출해주는 게이트웨이 패턴 / HttpOnly 쿠키에는 무작위 토큰만 담고 서버에는 그 해시만 저장해 신원 노출을 최소화하는 설계

### share · `share`
파일을 끌어다 놓으면 링크가 생기고, 24시간 뒤 자동으로 사라지는 임시 파일 공유 서비스.
- 드래그 앤 드롭으로 최대 10GB 파일을 업로드하고 짧은 링크를 즉시 발급
- 90MB를 기준으로 한 방 업로드와 R2 멀티파트 업로드(50MB씩 4개 병렬, 조각당 3회 재시도)를 자동으로 갈라 처리
- 매시 정각 cron과 링크 접근 시점 이중 체크로 24시간 지난 파일을 자동 삭제
- **스택**: Cloudflare Workers · Cloudflare R2 · TypeScript · Vitest
- **🎓 배울 수 있는 것**: 데이터베이스 없이 R2 오브젝트의 customMetadata만으로 파일명·만료 시각을 관리하는 설계 / Worker 요청 본문 한도를 우회하기 위해 파일 크기에 따라 단일/멀티파트 업로드 경로를 분기하는 패턴

### 헌책방나우 · `kr-oldbook-now`
지금 이 시각 문을 연 전국 헌책방·독립서점·북카페 1,677곳을 찾는 빌드 도구 없는 정적 웹앱.
- 한국문화정보원 공공 API 3종(중고서점·독립서점·북카페)을 수집해 이름+150m 이내 중복을 병합
- 요일별 영업시간과 정기 휴무를 대조해 '지금 영업 중'·'곧 마감'을 판정, 정보 없음은 휴무로 단정하지 않고 '시간 미상'으로 분리
- 카카오맵은 좌표로 이름표 핀을 직접 찍고, 네이버지도는 주소+좌표 중심 고정으로 검색 실패를 우회
- **스택**: Vanilla JS · Node.js · Leaflet · 한국문화정보원(KCISA) 공공 API
- **🎓 배울 수 있는 것**: 엑셀 시리얼 값·손상된 필드(좌표 문자열이 시각 자리에 낀 원본 데이터)를 감지해 추정 복원하고, 추정치는 'reconstructed' 신뢰도 배지로 정직하게 표시하는 법 / 빌드 도구 없이 브라우저·Node 양쪽에서 그대로 돌아가는 순수 ES 모듈로 앱을 구성하는 법

### ShortFlow · `shortflow`
브라우저에서 완결되는 유튜브 쇼츠(9:16, 최대 60초) 편집기 — 영상이 서버로 올라가지 않고 ffmpeg.wasm이 클라이언트에서 인코딩한다.
- 파일 드롭·유튜브 URL로 영상을 가져와 트림·9:16 리프레임·배경음악 트랙까지 편집
- 자막 큐를 타임라인 드래그·핸들로 길이 조절하고 화면에서 위치·크기·색상·정렬을 바로 조정
- 1080×1920 H.264/AAC MP4로 내보내기, 프리뷰와 동일한 Canvas 렌더러로 자막을 PNG로 구워 overlay
- **스택**: Next.js 16 · React 19 · TypeScript · Zustand · ffmpeg.wasm · Tailwind CSS v4
- **🎓 배울 수 있는 것**: ffmpeg.wasm은 실시간 프리뷰에 못 쓴다는 제약을 인정하고, 프리뷰(video+Canvas 합성)와 익스포트(ffmpeg)를 분리 설계하는 법 / 프로젝트 상태 → ffmpeg argv 변환을 부수효과 없는 순수 함수로 만들어 wasm 없이 전부 테스트하는 법

### 약국 나우 · `datalab/kr-pha-now`
지금 문을 연 약국을 내 주변 지도와 목록에서 바로 찾는 웹앱.
- 국립중앙의료원 전국 약국 정보와 한국천문연구원 특일 정보를 대조해 공휴일까지 반영한 '지금 영업 중' 판정
- 지도를 옮기면 그 위치 주변 약국을 다시 불러오고, 데스크탑에서는 지도·목록을 좌우로 나눠 보여줌
- 초기 진입 시 핀이 하나도 안 찍히던 문제를 고쳐, 위치 권한 없이도 지도 이동만으로 탐색 가능
- **스택**: Next.js 16 · React 19 · TypeScript · Tailwind CSS · 공공데이터포털(국립중앙의료원 약국정보·특일정보)
- **🎓 배울 수 있는 것**: 영업시간 API와 공휴일 API를 조합해 '지금 여는지'를 판정하는 로직 설계 / 지도 뷰포트가 바뀔 때마다 그 범위의 데이터만 다시 불러오는 nearby 쿼리 패턴

### 클릭파워 · `clickpower`
ClickHouse 하나로 저장·조회까지 끝내는 가벼운 ELK 대체 로그 플랫폼.
- Vector로 로그를 흘려보내면 ClickHouse에 그대로 쌓고, 로그 탐색기(Discover)에서 검색·필터링
- `level:error service:api "connection timeout" -host:web-3` 같은 DSL 검색 문법을 SQL로 변환해 실행
- 모든 사용자 입력값은 파라미터 바인딩으로 처리해 SQL 인젝션 없이 안전하게 쿼리 실행
- **스택**: Next.js 16 · ClickHouse · Vector · TypeScript · Docker Compose
- **🎓 배울 수 있는 것**: 검색 DSL을 파라미터 바인딩된 SQL로 안전하게 변환하는 파서 설계 / ClickHouse를 로그 저장소로 쓸 때의 스키마 설계(JSON 컬럼 attributes로 임의 필드 수용)

### 부산나우 · `kr-busan-now`
부산 여행자가 도착해 오늘 뭘 할지 바로 정하도록 도와주는 5개 언어 여행 정보 웹앱.
- 부산관광공사 비짓부산 API로 오늘 열리는 축제·공연·전시·크루즈 입항을 홈에서 바로 보여줌
- 9개 카테고리 탐색과 갈맷길 9코스·비짓부산패스 추천 일정 12개를 지도로 제공
- 번역이 아니라 언어별로 직접 쓰인 원문(한/영/일/중국어 간·번체)을 그대로 노출
- **스택**: Next.js 16 · TypeScript · Tailwind CSS · 비짓부산 API · 공공데이터포털 · Vercel
- **🎓 배울 수 있는 것**: 번역기를 거치지 않고 언어별 원문 오퍼레이션을 그대로 서빙하는 다국어 설계 / 정적 스냅샷이라도 '오늘·이번 주' 판정은 요청 시점 KST 벽시계로 계산하는 법

### 식약처나우 · `kr-mfds-now`
식약처 공개데이터 전부를 한곳에서 검색·조회하는 웹서비스.
- 식약처가 공개하는 데이터셋 전체를 카탈로그로 만들어 검색·탐색 가능하게 정리
- 의약품·화장품 등 항목별 상세 조회와 이미지 표시 지원
- Pretendard 폰트에 화이트&블루 톤으로 가독성 위주 UI 구성
- **스택**: Next.js 16 · TypeScript · Tailwind CSS · Pretendard · 식약처 공공데이터 · Vercel
- **🎓 배울 수 있는 것**: 수십 개 공공 데이터셋을 자동으로 카탈로그화하는 스크립트 설계(scripts/build-catalog.py) / 출처가 제각각인 정부 데이터를 하나의 검색 UI로 통합하는 법

### 상권나우 · `datalab/kr-market-now`
1년 사이 우리 동네에 생긴 가게와 사라진 가게를 지도로 보는 앱.
- 소상공인시장진흥공단 상가업소 데이터 두 분기(202506 vs 202606)를 비교해 신규·소멸·간판 교체 매장을 분류
- 같은 자리·같은 업종이 이름만 바뀐 경우(개명)와 단독 건물 매장의 간판 교체까지 여러 단계로 재매칭해 오탐 제거, 동네 검색으로 반경 안 변화만 필터링
- **스택**: Next.js 16 · MapLibre GL · pmtiles · Vercel
- **🎓 배울 수 있는 것**: 공공데이터의 상가업소번호 재부여 문제를 다단계 매칭으로 보정하는 법 / 대용량 지오데이터를 pmtiles로 압축해 서버 없이 지도에 뿌리는 법

### 100년 잔디 · `lifelog`
인생 100년을 1,200칸의 잔디밭으로 보여주는 타임라인.
- 생년월일을 넣으면 지금까지 산 개월 수와 남은 개월 수를 1,200칸(100년×12개월) 격자로 시각화
- 인생의 사건을 직접 추가해 타임라인에 기록하고, 42세에 감독 데뷔한 클린트 이스트우드 같은 '늦은 시작' 사례와 비교
- 서버 없이 URL 해시로 상태를 인코딩해 공유 링크를 만들고, 캔버스로 PNG 포스터도 저장
- **스택**: Vanilla JS · HTML/CSS Canvas · LocalStorage · Vercel
- **🎓 배울 수 있는 것**: 프레임워크 없이 상태·렌더링을 직접 관리하는 바닐라 JS 패턴 / 서버 없이 URL 해시 인코딩만으로 상태를 공유하는 방법

### 육아나우 · `kr-baby-now`
지금 문 연 소아과·산부인과부터 산후조리원·놀이터·아이돌봄 기관까지 내 주변 육아 시설을 지도로 찾는 웹앱.
- 국립중앙의료원·질병관리청 데이터로 야간·주말·공휴일 진료 중인 소아청소년과·산부인과와 국가예방접종 의료기관을 보여줌
- 산후조리원, 공개·유료 실내 놀이터, 아이돌봄 기관, 부산 어린이집·수유실을 레이어별로 지도에 표시
- 런타임 API 호출 없이 정적 스냅샷을 브라우저에서 거리순 정렬·필터링
- **스택**: Next.js 16 · TypeScript · Tailwind CSS · Python · 공공데이터포털 · Vercel
- **🎓 배울 수 있는 것**: 공공데이터를 미리 수집·가공해 정적 JSON으로 배포하면 API 키와 호출 한도 걱정 없이 빠르게 서비스할 수 있다는 것 / 좌표계(EPSG:5174→WGS84) 변환과 이름+시군구 조인으로 서로 다른 출처의 데이터를 합치는 법

### 금융나우 · `kr-finance-now`
금융위원회 공공데이터 오픈API 110종을 한 사이트에서 검색하고 바로 조회하는 웹서비스.
- 금융위원회 오픈API 110종(조회 349개)을 카탈로그로 정리해 검색·탐색할 수 있게 함
- 카탈로그 JSON 한 벌로 데이터셋마다 조회 폼과 결과 표를 자동 생성
- 홈 화면에서 코스피·코스닥·코스피200 주요 지수를 보여주고 응답 필드에 공식 한글 라벨을 병합
- **스택**: Next.js 16 · TypeScript · Tailwind CSS · Python · 공공데이터포털 · Vercel
- **🎓 배울 수 있는 것**: API 명세를 데이터(카탈로그)로 만들면 화면을 하나하나 짜지 않아도 수백 개 API를 같은 UI로 다룰 수 있다는 것 / JSON·XML·BOM·게이트웨이 오류처럼 제각각인 응답을 한 모양으로 파싱하는 방어적 처리
...
### 선물 스캔 대시보드 · `futures-scan`
바이낸스 선물 714종목에서 과매도+거래량 급증 종목을 찾아 백테스트까지 자동으로 굴리는 정적 대시보드.
- RSI(14)<30 이면서 거래량이 20봉 평균의 3배 이상인 종목을 매시간 스캔
- 히트 종목마다 차트·지지선·백테스트·캔들 단위 리플레이 페이지를 자동 생성, 서버 크론이 결과를 정적 사이트로 만들어 배포 — 실시간 서버 비용 0
- **스택**: Python 3.12 · ccxt · pandas·parquet · TradingView Lightweight Charts · Vercel
- **🎓 배울 수 있는 것**: 백테스트로 전략을 검증하는 기본 절차와 그 한계(슬리피지·펀딩 미반영) / 배치가 만든 결과를 정적 사이트로 배포하는 값싼 운영 패턴

### 공공데이터 신규 API 알리미 · `datalab/alert-data`
공공데이터포털에 새로 올라온 오픈API를 감지해 디스코드·텔레그램으로 알려주는 서버 봇.
- 매시간 폴링해 본 적 없는 API id만 신규로 판정(state.json에 누적 기록), 상세 페이지까지 읽어 REST 여부·좌표 포함 여부를 확인해 "떴는데 쓸 수 있나"까지 판정
- 전송 실패 시 상태를 전진시키지 않고, 60건 넘게 신규로 잡히면 오탐으로 보고 발송을 멈춤
- **스택**: Python 3.12 · Discord Webhook · Telegram Bot API · systemd user timer (ebs 서버)
- **🎓 배울 수 있는 것**: 정렬값 하나(sort=date) 때문에 오탐이 쏟아질 수 있다는 것 — 기본 랭킹은 매 요청 순서가 흔들린다 / 상태 파일은 임시파일에 쓰고 rename해야 중단된 실행이 다음 실행을 막지 않는다

### 열차 간편 조회 (코레일) · `korail`
코레일 KTX·ITX·무궁화 잔여석을 조회하고 클릭 한 번으로 결제 전 좌석을 확보하는 개인용 도구.
- 공식 조회 API가 안티 매크로로 막힌 뒤에도 동작하는 비공식 모바일 API로 조회·좌석 확보(결제 전 홀드)까지 처리하고, 결제·취소·환불 코드는 테스트로 원천 차단
- 매진 좌석은 SQLite 감시 목록에 등록해 로컬 서버가 15초마다 확인하다 빈 자리가 나는 즉시 확보하고 텔레그램으로 알림
- launchd로 구간별 좌석 감시와 결제 마감 10/5/3/1분 전 알림을 자동화
- **스택**: Python (FastAPI) · Vercel · SQLite · Telegram Bot API · launchd
- **🎓 배울 수 있는 것**: 안티 매크로 토큰(DynaPath)을 지원하는 대체 라이브러리로 갈아타 API 차단을 우회한 사례 / '취소·결제·환불 코드가 들어오면 테스트가 실패한다'처럼 안전 규칙을 코드가 아니라 테스트로 강제하는 설계
