...
> 1년 동안 100개의 서비스를 만드는 프로젝트 — 지금까지 75개
...
| 분야 | 프로젝트 수 | 대표작 |
|------|:---:|------|
| 🌐 웹 서비스·대시보드 | 36 | 국회 감시 대시보드, 법령 검색, 실시간 같이보기(Unflix) |
...
### 서울 실시간 혼잡도 지도 · `datalab/seoul-vibe`
서울 121장소의 실시간 인구 혼잡도를 지도에 얹고, 장소를 고르면 날씨·대기질·도로소통·주차장·따릉이·문화행사를 한 화면에서 보여주는 대시보드.
- 서울 열린데이터광장 실시간 도시데이터로 121장소 혼잡도를 지도 위에 색으로 표시
- 평문 HTTP·비표준 포트인 서울 API를 서버 라우트로 감싸 프록시
- **스택**: Next.js · MapLibre GL · 서울 열린데이터광장 API
- **🎓 배울 수 있는 것**: 예비 인증키를 트래픽 초과 시에만 넘기는 폴백 설계 (fetch 캐시 히트율 유지) / 레거시 평문 HTTP API를 안전하게 서버 라우트로 감싸는 법

### 산행 — 전국 등산로 지도 · `datalab/korea-mountain`
산림청 100대명산·국립공원 탐방로 등 전국 등산 공공데이터를 지도 위에서 훑어보는 웹앱.
- 100대명산 607코스, 전국 4,492봉우리, 국립공원 탐방로 1,890구간을 난이도·소요시간과 함께 지도에 표시
- 산악기상정보·산불위험예보를 실시간 API로 얹어 표시
- **스택**: Next.js · MapLibre GL · 브이월드 · 공공데이터포털 API
- **🎓 배울 수 있는 것**: 좌표 없는 원시 공공데이터를 GeoJSON으로 변환하는 파이프라인 / 기상청 격자 좌표계 변환으로 지점별 산악기상 조회하기
### 입양 — 전국 유기동물 공고 · `datalab/ipyang`
전국 동물보호센터의 구조동물 공고를 '마감이 임박한 순서'로 보여줘, 지금 도움이 급한 아이부터 눈에 띄게 하는 웹앱.
- 최신순으로만 주는 원본 API 대신 공고를 전부 받아 남은 날짜로 다시 정렬해 보여줌
- 평문 HTTP로 오는 정부 사진 서버를 서버 라우트로 감싸 CDN에 캐시
- **스택**: Next.js · TypeScript · Tailwind CSS · shadcn/ui · 공공데이터포털 국가동물보호정보시스템 API
- **🎓 배울 수 있는 것**: 원본 API의 정렬을 믿을 수 없을 때 전수 수집 후 서버에서 다시 정렬하기 / mixed content와 잘못된 content-type을 이미지 프록시로 바로잡는 법

### gofish — 전국 바다낚시지수 · `datalab/gofish`
전국 49개 예보지점의 바다낚시지수를 7일치로 보는 지도. 오늘 어디가 좋은지는 랭킹으로, 이번 주 흐름은 지점별 추이로 본다.
- 국립해양조사원 바다낚시지수 1,750건을 지점·날짜·어종별로 지도와 랭킹에 표시
- 물때·파고·수온·기온·유속·풍속을 지점마다 함께 보여줌
- **스택**: Next.js · TypeScript · MapLibre GL · Tailwind CSS · 공공데이터포털 바다낚시지수 API
- **🎓 배울 수 있는 것**: 페이지당 100건 상한이 있는 공공 API를 통째로 받아 두고 메모리에서 거르는 캐시 설계 / 지도 글리프 서버에 한글이 없어 라벨이 사라질 때 브라우저 폰트로 그리게 하는 법

### 멍냥로드 — 반려동물 동반여행 지도 · `datalab/pet-travel`
반려동물과 함께 갈 수 있는 전국 관광지·카페·숙소를 지도에서 찾고, 동반 유형(전구역/일부구역)으로 걸러 헛걸음을 줄이는 앱.
- 한국관광공사 TourAPI 기반 전국 9,686곳을 지역·분류·검색으로 탐색
- 동반 유형(전구역/일부구역 가능)으로 필터링, 결측은 '정보 없음'으로 정직하게 표시
- **스택**: Next.js · TypeScript · Tailwind v4 · shadcn/ui · MapLibre GL · 한국관광공사 TourAPI
- **🎓 배울 수 있는 것**: 전량 정적 번들 + 필요한 지점만 런타임 폴백하는 하이브리드로 일일 쿼터 분산 / 결측을 '불가'가 아니라 '정보 없음'으로 다루는 데이터 설계

### 공항 지금 — 전국 공항 운항 보드 · `datalab/kr-airport-now`
전국 공항의 항공편 운항 정보와 인천공항의 주차·출국장 실시간 현황을 한 화면에서 보여주는 모바일 웹앱.
- 한국공항공사 전국 운항현황으로 공항별 도착·출발 항공편을 한눈에
- 인천은 자리 많은 주차장·대기 짧은 출국장을 터미널(T1·T2)별로, 모든 값에 관측 시각을 함께 표시
- **스택**: Next.js · TypeScript · Tailwind v4 · shadcn/ui · 공공데이터포털 API
- **🎓 배울 수 있는 것**: 타임존 표기 없는 KST 벽시계 값을 UTC 인스턴트로 확정해 9시간 어긋남 막기 / 값 0의 의미(미운영 vs 실제 0)를 구분해 단언하지 않고 보여주기

### 한국 축제·행사 파인더 · `datalab/kr-events-now`
지금~2주 안에 내 주변에서 열리는 한국의 축제·행사를 영어·일본어·중국어(간체/번체)로 지도와 리스트로 보여주는 외국인 여행객용 앱.
- 한국관광공사 다국어 관광정보서비스로 4개 언어별 축제 목록을 매일 한 번 가져와 캐싱
- 위치 기반 거리순 정렬은 브라우저에서 계산해 언어당 일일 쿼터를 아낌, 결측 정보는 '정보 없음'으로 정직하게 표시
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · shadcn/ui · MapLibre GL v5 · 한국관광공사 다국어 TourAPI
- **🎓 배울 수 있는 것**: 다국어 API를 언어별로 서버에서 하루 한 번만 캐싱하고 위치 필터링은 클라이언트로 옮겨 쿼터를 아끼는 법 / 외국인 대상 서비스에서 결측 데이터를 기계번역하지 않고 '정보 없음'으로 정직하게 보여주는 설계

### 택스리펀 지금 · `datalab/kr-taxfree-now`
내 주변에서 택스리펀(사후면세) 되는 가게를 영어·일본어·중국어(간체/번체)로 지도에서 찾는 외국인 여행객용 앱.
- 한국관광공사 다국어 관광정보서비스(쇼핑 카탈로그, contentTypeId=79)를 언어별로 하루 한 번 전량 수집해 메모리 캐싱, 요청 좌표로 가까운 200건만 응답
- 브랜드 칩을 데이터에서 자동 추출(올리브영·롯데하이마트·아트박스 등), 환급률·금액은 계산하지 않고 태그 유무만 정직하게 표시
- **스택**: Next.js 16 · TypeScript · Tailwind v4 · shadcn/ui · MapLibre GL v5 · 한국관광공사 다국어 TourAPI
- **🎓 배울 수 있는 것**: 전량 데이터를 서버가 하루 1회 캐싱하고 요청마다는 nearest 만 골라 내려, 위치가 바뀌어도 업스트림 호출을 0으로 유지하는 설계 / 태그 없음("정보 없음")과 미해당을 구분해 돈이 걸린 정보를 과장하지 않는 법
