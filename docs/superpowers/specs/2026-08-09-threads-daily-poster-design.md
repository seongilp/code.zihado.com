...
`projects.json`에 정리된 프로젝트(설계 시점 60개, 주간 자동 업데이트로 계속 늘어난다)를
매일 하나씩 Threads에 소개한다.
...
| 항목 | 결정 | 근거 |
|---|---|---|
| 문구 생성 | headless `claude -p`로 매번 새로 작성 | 기존 `auto-update.sh`와 같은 패턴. 전부 같은 틀이면 피드에서 단조롭다 |
